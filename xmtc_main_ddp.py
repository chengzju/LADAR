...
from models.MADModel import MADModel
...
def main():
    ...
    parser.add_argument('--sample_test_size', type=int, default=2000)
    ...
    if args.sample_test and args.train_model:
        test_data = random.sample(test_data, args.sample_test_size)
    ...
    if args.train_model:
        ...
        model.train(encoder=encoder, model=models, optimizer=optimizer,
                    ...
                    label2id=label2id, label_cnt=train_labels, device=device)
    if args.test_model:
        ...
        models = [network_layer, network_granu]
        true_labels, outputs = model.predict(encoder, models, test_loader, device, 10)
